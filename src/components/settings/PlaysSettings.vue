      ...
      <div class="input-group-append">
        <span>
          ...
          <div class="dropdown-menu" :class="{ show: dropdownOpen }">
            <a v-for="owner in ownersWithPlays " :key="owner" class="dropdown-item " href="javascript:void(0);" @click="setSelectedOwner(owner) ">{{ owner }}</a>
          </div>
        </span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    setSelectedOwner(owner) {
      const oldOwner = this.selectedOwner;
  ...
  created() {
    window.addEventListener('click', (evt) => {
      const elem = document.getElementById('playsDropdownButtonGroup');
