  ...
  <div class="flex-page-item-grow">
    <div class="container-fluid tbody" ref="nonSimpleBarScroller">
      <div class="row">
        ...
        <div v-else v-for="(item) in filteredItems" :key="item.id" class="col-6 col-sm-4 col-md-4 col-lg-2 text-center card-parent">
          <div class="card">
            <div class="rank">{{item | filterItemRank}}</div>
            <label for="modal-switch" class="show-game-modal" role="button" data-toggle="modal" data-target="#myModal" @click="$emit('show-modal', item)">
...
export default {
  ...
  methods: {
    ...
    updateCards() {
      const cards = this.$el.querySelectorAll('.card-parent');
      // let card; let item;
      let i;
      ...
      const minBottom = (scrollerTop - scrollerHeight);
      const maxTop = (scrollerTop + (2 * scrollerHeight));
