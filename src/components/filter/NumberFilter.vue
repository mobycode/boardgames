...
export default {
  ...
  methods: {
    matches(item) {
      let matches = true;
      const valueProperty = this.action.valueProperty || this.valueProperty;
      const itemValue = item[valueProperty];
      const itemMinValue = item[this.minValueProperty || this.valueProperty];
      const itemMaxValue = item[this.maxValueProperty || this.valueProperty];
    ...
    focusInput(evt) {
      if (!evt || evt.target.checked) {
        const elem = this.$el.querySelector('input[type=number]:not(:disabled)');
        if (elem) {
          elem.focus();
        }
      }
    ...
    fromQuery() {
      ...
      const parser = Number[this.valuePrecision === 0 ? 'parseInt' : 'parseFloat'];
      const parseNumber = (val) => (val ? parser(val, 10) : undefined);
