...
<div class="row" :data-filtered="filtered">
  <div class="col">
    <div class="input-group" :class="{'input-group-sm': deviceSizeValue < 2, 'input-group-xs': deviceSizeValue < 0}">
      ...
      <input placeholder="Min" v-if="valueCount === 2" v-model.number="minValue" type="number" step="0.01" class="form-control" :class="{'is-invalid': enabled && $v.minValue.$error}" @input="$v.minValue.$touch(); autoEnable($event);" :disabled="valueCount !== 2">
      <div class="input-group-append input-group-insert" v-if="valueCount === 2">
        <div class="input-group-text">{{ andText }}</div>
      </div>
      <input placeholder="Max" v-if="valueCount === 2" v-model.number="maxValue" type="number" step="0.01" class="form-control" :class="{'is-invalid': enabled && $v.maxValue.$error}" @input="$v.maxValue.$touch(); autoEnable($event);" :disabled="valueCount !== 2">
      <input placeholder="Weight" v-if="valueCount === 1" v-model.number="value" type="number" step="0.01" class="form-control" :class="{'is-invalid': enabled && $v.value.$error}" @input="$v.value.$touch(); autoEnable($event);" :disabled="valueCount !== 1">
    </div>
  </div>
</div>
</template>
